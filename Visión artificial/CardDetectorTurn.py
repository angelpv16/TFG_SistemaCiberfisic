...
while cam_quit == 0:
    ...
    if len(cnts_sort) != 0:
        ...
        for i in range(len(cnts_sort)):
            if (cnt_is_card[i] == 1):
                ...
                with open('suit_rank_Turn.txt', 'w') as f:
    ...
    if k == 1 and cards[0].best_rank_match  and cards[0].best_suit_match:
        ...
        contenidoTurn = (cards[0].best_rank_match, cards[0].best_suit_match)

        contenidoTurn = list(contenidoTurn)  # Convertimos la tupla a lista para poder modificar los elementos

        for i in range(len(contenidoTurn)):
            for palabra, inicial in mapeo_iniciales.items():
                if contenidoTurn[i] == palabra:
                    contenidoTurn[i] = inicial
                elif isinstance(contenidoTurn[i], str):
                    contenidoTurn[i] = contenidoTurn[i].replace(palabra, inicial)

        contenidoTurn = "".join(contenidoTurn)  # Convertimos la lista nuevamente a cadena de texto
