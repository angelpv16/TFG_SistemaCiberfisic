...
for palabra, inicial in mapeo_iniciales.items():
    if inicial.isdigit():
        contenidoHand = contenidoHand.replace(palabra, inicial)
    else:
        contenidoHand = contenidoHand.replace(palabra, inicial[0])

# Busca las ocurrencias de ":" en el contenidoHand y se queda solo con las letras o números después de los ":"
nuevo_contenidoHand = ""
for linea in contenidoHand.split("\n"):
    if ":" in linea:
        nuevo_contenidoHand += linea.split(":")[1]
    else:
        nuevo_contenidoHand += linea

# Separar cada dos letras o números por ";"
contenidoHand_final = ""
for i in range(0, len(nuevo_contenidoHand), 2):
    contenidoHand_final += nuevo_contenidoHand[i:i + 2]

# Abre el archivo en modo escritura para guardar los cambios
with open("suit_rank.txt", "w") as archivo:
    archivo.write(contenidoHand_final)
